...
class Parts {
  ...
  /* سلاسل معروفة: أي عنوان يبدأ بأحدها يُعتبر من نفس السلسلة */
  ...
  static String base(Movie m) {
    ...
    // حذف الترجمة/العنوان الفرعي بعد النقطتين
    t = t.split(':').first;
    // حذف السنوات
...
class Taste {
  ...
  static Map<String, dynamic> analyze() {
    ...
    for (final m in h) {
      ...
      if (m.year > 0) d[(m.year ~/ 10) * 10] = ((m.year ~/ 10) * 10) + 0 + (d[(m.year ~/ 10) * 10] ?? 0);
...
class TasteScreen extends StatelessWidget {
  ...
  Widget _row(String l, String v) => Card(color: const Color(0xFF151B23), margin: const EdgeInsets.only(bottom: 8),
      child: ListTile(dense: true, title: Text(l, style: const TextStyle(fontSize: 13)),
          trailing: Text(v, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: AppTheme.accent))));
}
...
class Exporter {
  ...
  /* 📥 يفتح مدير الملفات مباشرة ويستورد تلقائياً */
